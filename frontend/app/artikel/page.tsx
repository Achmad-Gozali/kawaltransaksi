...
export const metadata: Metadata = {
  ...
  description: 'Artikel dan analisis pola penipuan online terbaru di Indonesia.',
};
...
function formatLoss(amount: number): string {
  ...
  return new Intl.NumberFormat('id-ID', { notation: 'compact', maximumFractionDigits: 1, style: 'currency', currency: 'IDR' }).format(amount);
}
        ...
        <div className="max-w-5xl mx-auto">
          {!articles || articles.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-sm font-black text-slate-400 uppercase tracking-widest mb-2">Belum ada artikel</p>
              <p className="text-xs text-slate-400">Artikel pertama akan muncul otomatis minggu depan.</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-100">
                      ...
                      <div className="flex gap-1.5">
                        {(article.total_reports ?? 0) > 0 && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">{article.total_reports} laporan</span>
                        )}
