...
const ewalletLogoMap: Record<string, string> = {
  gopay: "/ewallets/gopay.png", dana: "/ewallets/dana.png",
  ovo: "/ewallets/ovo.png", shopeepay: "/ewallets/shopeepay.png",
          ...
          <div className="flex items-start gap-4 sm:gap-6 flex-wrap sm:flex-nowrap">
                ...
                {[
                  ...
                  <Link key={t.val} href={buildUrl({ type: t.val })} scroll={false}
                    className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors ${
                      type === t.val ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-500 border-slate-200 hover:border-slate-300"
                    }`}>
                    {t.label}
                  </Link>
                ))}
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-1.5">
                {[{ val: "latest", label: "Terbaru" }, { val: "oldest", label: "Terlama" }].map((s) => (
                  <Link key={s.val} href={buildUrl({ sort: s.val })} scroll={false}
                    className={`px-3 py-1.5 text-xs font-semibold rounded-lg border transition-colors ${
                      sort === s.val ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-500 border-slate-200 hover:border-slate-300"
                    }`}>
                    {s.label}
                  </Link>
                ))}
            ...
            <div className="sm:ml-auto flex items-end pb-0.5 gap-3">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{totalUniqueNumbers} nomor unik</span>
              {(q || type !== "all" || sort !== "latest") && (
                <Link href="/laporan-publik" className="text-xs text-slate-400 hover:text-red-500 font-semibold transition-colors">Reset</Link>
              )}
            </div>
          </div>
          {q && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-md font-semibold">&quot;{q}&quot;</span>
            </div>
          )}
        </div>
      </section>
                ...
                return (
                  <Link key={report.target_number} href={`/check/${encodeSlug(report.target_number)}`}
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest rounded-md border ${badge.className}`}>{badge.label}</span>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {Number(report.total) > 1 && (
                          <span className="text-[10px] font-bold text-red-500 uppercase tracking-widest shrink-0">{report.total} laporan</span>
                        )}
                        {Number(report.total) === 1 && (
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate min-w-0">{report.category}</span>
                        )}
                      </div>
          ...
          {totalPages > 1 && (
            <div className="mt-8 sm:mt-10">
              ...
              <div className="flex justify-center items-center gap-1.5 flex-wrap">
                {safePage > 1 ? (
                  <Link href={buildUrl({ page: String(safePage - 1) })} scroll={false} className="px-3 py-2 text-xs font-bold border border-slate-200 rounded-lg hover:border-slate-400 hover:bg-slate-50 transition-all">&larr;</Link>
                ) : (
                  <span className="px-3 py-2 text-xs font-bold border border-slate-100 rounded-lg text-slate-300 cursor-not-allowed">&larr;</span>
                )}
                  ...
                  p === "..." ? (
                    ...
                    <Link key={p} href={buildUrl({ page: String(p) })} scroll={false}
                      className={`w-9 h-9 flex items-center justify-center text-xs font-bold rounded-lg border transition-all ${
                        p === safePage ? "bg-slate-900 text-white border-slate-900" : "border-slate-200 text-slate-600 hover:border-slate-400 hover:bg-slate-50"
                      }`}>
                      {p}
                    </Link>
                  )
                ...
                {safePage < totalPages ? (
                  <Link href={buildUrl({ page: String(safePage + 1) })} scroll={false} className="px-3 py-2 text-xs font-bold border border-slate-200 rounded-lg hover:border-slate-400 hover:bg-slate-50 transition-all">&rarr;</Link>
                ) : (
                  <span className="px-3 py-2 text-xs font-bold border border-slate-100 rounded-lg text-slate-300 cursor-not-allowed">&rarr;</span>
                )}
              </div>
            </div>
          )}
