...
interface EwalletData {
  name: string; fullName: string; logo: string;
  callCenter: string; website: string; description: string;
}
...
const SOCIALS: Record<string, { instagram?: string; tiktok?: string }> = {
  GoPay:     { instagram: "https://www.instagram.com/gopayindonesia?igsh=amo1enhtbHRocnAx",   tiktok: "https://www.tiktok.com/@gopayindonesia?_r=1&_t=ZS-95BnWJjSmDV" },
  DANA:      { instagram: "https://www.instagram.com/dana.id?igsh=MXNwa2JyN29temV6Zg==",     tiktok: "https://www.tiktok.com/@dana.indonesia?_r=1&_t=ZS-95BnaGLA0N2" },
  LinkAja:   { instagram: "https://www.instagram.com/linkaja?igsh=ejNjdjg4c3R4YnY4",          tiktok: "https://www.tiktok.com/@linkajaid?_r=1&_t=ZS-95BnjrE9TnA" },
  ShopeePay: { instagram: "https://www.instagram.com/shopee_id?igsh=MW9sa2hsc2JiMWZ1bA==",   tiktok: "https://www.tiktok.com/@shopee_id?_r=1&_t=ZS-95Bo2gL2ZGe" },
  OVO:       { instagram: "https://www.instagram.com/ovo_id?igsh=MWZiczE3NnYybDQ1Zw==",      tiktok: "https://www.tiktok.com/@ovo.id?_r=1&_t=ZS-95BoCLQZGOT" },
};
          ...
          <div className="grid lg:grid-cols-3 gap-8 items-start">
            <div className="lg:col-span-2">
              ...
              <div className="flex flex-wrap gap-2">
                <a href={data.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 text-white text-xs font-medium rounded-lg hover:bg-slate-800 transition-colors">
                  <Globe className="w-3.5 h-3.5" /> Kunjungi website
                </a>
                <a href={`tel:${data.callCenter}`} className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 text-xs font-medium rounded-lg hover:border-slate-300 hover:bg-slate-50 transition-colors shadow-sm">
              ...
              <div className="lg:col-span-1">
                <div className="bg-white rounded-[8px] border border-slate-200/80 shadow-sm overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-100">
                    <p className="text-[10px] text-slate-400 uppercase tracking-[0.12em] font-medium">Akun resmi {data.name}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5 leading-relaxed">Pastikan hanya berinteraksi dengan akun terverifikasi berikut.</p>
                  </div>
                  <div className="divide-y divide-slate-100">
                    {socials.instagram && (
                      <a href={socials.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between px-4 py-3.5 hover:bg-slate-50/60 transition-colors group">
                        <div className="flex items-center gap-3">
                          <div className="w-7 h-7 bg-pink-50 border border-pink-100 rounded-[8px] flex items-center justify-center shrink-0 text-pink-500"><InstagramIcon /></div>
                          <div>
                    ...
                    {socials.tiktok && (
                      <a href={socials.tiktok} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between px-4 py-3.5 hover:bg-slate-50/60 transition-colors group">
                        <div className="flex items-center gap-3">
                          <div className="w-7 h-7 bg-slate-900 border border-slate-800 rounded-[8px] flex items-center justify-center shrink-0 text-white"><TiktokIcon /></div>
                          <div>
        ...
        <div>
          ...
          {reports.length > 0 ? (
                ...
                <tbody className="divide-y divide-slate-100">
                  {reports.map((report, i) => (
                    <tr key={i} className="group hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-semibold border ${report.status === "verified" ? "bg-red-50 text-red-700 border-red-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}>
                          {report.status === "verified" ? "Terverifikasi" : "Pending"}
                        </span>
                      </td>
                      <td className="px-5 py-4"><span className="text-[11px] text-slate-400">{report.dateFormatted}</span></td>
                      <td className="px-5 py-4 text-right">
                        <Link href={`/check/${encodeSlug(report.target_number)}?type=ewallet&wallet=${walletId}`} className="inline-flex items-center justify-center w-7 h-7 rounded-[8px] bg-slate-100 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all">
                ...
                {reports.map((report, i) => (
                  <Link key={i} href={`/check/${encodeSlug(report.target_number)}?type=ewallet&wallet=${walletId}`} className="flex items-center justify-between px-4 py-3.5 hover:bg-slate-50/60 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${report.status === "verified" ? "bg-red-50 text-red-700 border-red-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}>
                        {report.status === "verified" ? "Terverifikasi" : "Pending"}
                      </span>
            ...
            <div className="bg-white rounded-[8px] border border-slate-200/80 shadow-sm p-14 text-center">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">Sampai sekarang belum ada akun {data.name} yang dilaporkan. Tetap hati-hati saat bertransaksi.</p>
            </div>
          )}
        </div>
