  ...
  return blocks.map((block, i) => {
    ...
    if (trimmed.startsWith("## ")) {
      return (
        <h2
          key={i}
          className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight mt-10 mb-3"
        >
          {trimmed.replace("## ", "")}
        </h2>
      );
      ...
      return (
        <ul key={i} className="space-y-2 pl-5 my-2">
          {items.map((item, j) => (
            <li
              key={j}
              className="text-slate-700 text-sm sm:text-base leading-relaxed list-disc font-normal"
            >
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex gap-12">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-2xl sm:text-4xl font-black tracking-tighter text-slate-900 mb-6 leading-tight">
              {article.title}
            </h1>
            ...
            <div className="mt-12 rounded-2xl overflow-hidden border border-slate-200">
              <div className="bg-slate-900 px-6 py-7 sm:px-8 sm:py-8">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Link
                    href="/cek-nomor"
                    className="flex-1 sm:flex-none text-center px-5 py-3 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-bold rounded-xl transition-colors uppercase tracking-widest"
                  >
                    Cek Nomor HP
                  </Link>
                  <Link
            ...
            {others && others.length > 0 && (
              <div className="mt-16">
                <p className="text-xl font-black text-slate-900 tracking-tight mb-8">
                  Artikel Lainnya
                </p>
                  ...
                  {others.map((a) => (
                    ...
                    >
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        {a.top_category && (
                          <span className="text-[10px] font-bold text-emerald-600 mb-2">
                            {a.top_category}
                          </span>
                        )}
                        <p className="text-sm font-black text-slate-800 group-hover:text-emerald-600 transition-colors leading-snug line-clamp-2 flex-1">
                          {a.title}
                        </p>
