...
import { ArrowLeft, ArrowRight, Phone, Hash, Globe, ShieldCheck, ChevronRight } from "lucide-react";
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
          <nav aria-label="Breadcrumb" className="mb-5 sm:mb-6">
            <ol className="flex flex-wrap items-center gap-1.5 text-xs text-slate-400">
              ...
              <li>
                <Link href="/cek-rekening" className="hover:text-slate-700 transition-colors">Cek Rekening</Link>
              </li>
              <li aria-hidden="true"><ChevronRight className="w-3 h-3" /></li>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2.5 px-0.5">
            <h2 className="text-[10px] uppercase tracking-[0.15em] text-slate-400 font-medium">Rekening {data.name} yang pernah dilaporkan sebagai penipu</h2>
                  ...
                  {reports.map((report, i) => (
                    <tr key={i} className="group hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-5 py-4 text-right">
                        <Link href={`/check/${encodeSlug(report.target_number)}?type=bank&bank=${bankId}`} className="inline-flex items-center justify-center w-7 h-7 rounded-[8px] bg-slate-100 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all">
