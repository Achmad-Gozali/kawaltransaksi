...
import { Loader2, CheckCircle2, AlertCircle, Pencil, X, Lock, ChevronRight } from 'lucide-react';
    ...
    <div className="py-3 space-y-3">
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2">
        <input
          ...
          autoFocus
          maxLength={100}
          className="flex-1 px-3 py-2 text-sm bg-zinc-50 border border-zinc-200 rounded-lg text-zinc-900 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none transition-all"
        />
        <button
          type="submit"
          disabled={loading || !name.trim() || name.trim() === currentName}
          className="px-3 py-2 bg-emerald-600 text-white text-xs font-semibold rounded-lg hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center gap-1"
        >
          {loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : 'Simpan'}
        </button>
        <button type="button" onClick={cancel} className="p-2 text-zinc-400 hover:text-zinc-600 transition-colors">
          <X className="w-4 h-4" />
        </button>
      </form>
    </div>
      ...
      {sent
        ...
        : (
          <button
            onClick={handleClick}
            disabled={loading}
            className="text-xs font-semibold text-emerald-600 hover:text-emerald-700 disabled:opacity-50 transition-colors flex items-center gap-1"
          >
