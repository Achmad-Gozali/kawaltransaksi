    ...
    >
      <div className="grid grid-cols-2 sm:grid-cols-3 sm:divide-x divide-slate-100">
        <div className="px-5 py-5 sm:px-8 sm:py-6 border-r border-b sm:border-b-0 border-slate-100">
          <p className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Laporan</p>
          <p className="text-2xl sm:text-4xl font-black text-slate-900 leading-none tabular-nums">{stats.total}+</p>
          <p className="text-xs text-slate-500 mt-1.5 leading-snug">Kasus dilaporkan</p>
        </div>
        <div className="px-5 py-5 sm:px-8 sm:py-6 border-b sm:border-b-0 border-slate-100">
          <p className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Verified</p>
          <p className="text-2xl sm:text-4xl font-black text-emerald-700 leading-none tabular-nums">{stats.verified}+</p>
          <p className="text-xs text-slate-500 mt-1.5 leading-snug">Terbukti penipuan</p>
        </div>
        <div className="col-span-2 sm:col-span-1 px-5 py-5 sm:px-8 sm:py-6 sm:border-l border-slate-100">
          <p className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Kerugian</p>
          <p className="text-2xl sm:text-4xl font-black text-red-500 leading-none tabular-nums">{formatLoss(stats.totalLoss)}</p>
          <p className="text-xs text-slate-500 mt-1.5 leading-snug">Total kerugian dilaporkan</p>
        </div>
      </div>
    </motion.div>
  );
    ...
    <main className="bg-white text-slate-900 font-sans overflow-x-hidden">
      ...
      <section className="relative min-h-[500px] sm:min-h-[600px] flex items-stretch overflow-hidden bg-white">
        <div className="relative z-10 flex flex-col justify-center px-5 sm:px-12 md:pl-20 lg:pl-28 py-14 md:py-20 w-full md:w-[52%]">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link href="/cek-nomor" className="px-5 sm:px-6 py-3 sm:py-3.5 bg-slate-900 text-white font-bold text-xs sm:text-sm tracking-widest uppercase rounded-xl flex items-center justify-center gap-2 hover:bg-emerald-700 transition-colors">
              <Phone className="w-4 h-4" /> cek nomor hp
            </Link>
            <Link href="/cek-rekening" className="px-5 sm:px-6 py-3 sm:py-3.5 border-2 border-slate-200 text-slate-900 font-bold text-xs sm:text-sm tracking-widest uppercase rounded-xl flex items-center justify-center gap-2 hover:border-slate-900 transition-colors">
        ...
        <div className="hidden md:flex relative w-[48%] items-center justify-start bg-white overflow-hidden pl-4">
          <div className="relative w-[460px] h-[460px]">
            <Image src="/poster.png" alt="Ilustrasi keamanan transaksi digital" fill priority fetchPriority="high" className="object-contain" sizes="(max-width: 768px) 0px, 460px" />
          </div>
        </div>
      </section>
      ...
      <div className="relative bg-white hidden sm:block">
        ...
        <div className="absolute bottom-0 left-0 right-0 translate-y-1/2 px-4 sm:px-6 z-10">
          <div className="max-w-4xl mx-auto"><StatsCard stats={stats} /></div>
        </div>
      </div>
            ...
            {recentReports.map((report, i) => {
              ...
              return (
                <motion.div key={report.id} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}>
                  <Link href={`/check/${encodeSlug(report.target_number)}`} className="block bg-white border border-slate-200 p-4 sm:p-5 rounded-lg hover:border-slate-300 hover:shadow-md transition-all group h-full">
                    <div className="flex justify-between items-start mb-4 sm:mb-5">
                      <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest rounded-full border ${report.status === 'verified' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : report.status === 'withdrawn' ? 'bg-slate-100 text-slate-500 border-slate-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                    ...
                    <div className="mb-4 sm:mb-5">
                      ...
                      <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mt-1">a.n. {report.target_name || 'anonymous'}</p>
                    </div>
                    <div className="flex items-center justify-between pt-3 sm:pt-4 border-t border-slate-100">
                      <div className="flex items-center gap-2 overflow-hidden">
                        <span className={`flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-bold border transition-colors ${meta.bg} ${meta.color} ${meta.border}`}>
      ...
      <svg viewBox="0 0 1440 80" preserveAspectRatio="none" className="w-full block bg-white -mb-1" xmlns="http://www.w3.org/2000/svg">
        <path d="M0,0 C360,60 720,20 1080,50 C1260,65 1380,35 1440,45 L1440,80 L0,80 Z" fill="#f8fafc" />
      </svg>
