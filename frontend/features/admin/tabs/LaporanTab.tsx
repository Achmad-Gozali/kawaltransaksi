...
import {
  CheckCircle2, XCircle, Clock, Eye, ExternalLink, Phone, Building2,
  ChevronDown, ChevronUp, Loader2, Search, X, FilePen, Download,
  AtSign, ShieldAlert, UserX, Store, MapPin, Undo2, Bot,
} from "lucide-react";
...
const ROBOT_SCORE_COLOR = (score: number) =>
  score >= 60 ? "bg-emerald-50 text-emerald-600 border-emerald-200"
  : score >= 30 ? "bg-amber-50 text-amber-600 border-amber-200"
  : "bg-red-50 text-red-500 border-red-200";

const ROBOT_BAR_COLOR = (score: number) =>
  score >= 60 ? "bg-emerald-500" : score >= 30 ? "bg-amber-400" : "bg-red-400";

const ROBOT_TEXT_COLOR = (score: number) =>
  score >= 60 ? "text-emerald-600" : score >= 30 ? "text-amber-600" : "text-red-500";

// ─── Primitives ──────────────────────────────────────────────────────────────

function Badge({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded-lg font-medium ${className}`}>
      {children}
    </span>
  );
}
...
export default function LaporanTab({ reports, initialSearch = "" }: { reports: Report[]; initialSearch?: string }) {
  ...
  const uniqueBanks = useMemo(
    () => Array.from(new Set(reports.map((r) => r.bank_name).filter(Boolean) as string[])).sort((a, b) => a.localeCompare(b)),
    [reports],
  );

  const uniquePlatforms = useMemo(
    () => Array.from(new Set(reports.map((r) => r.platform).filter(Boolean) as string[])).sort((a, b) => a.localeCompare(b)),
    [reports],
  );
  ...
  const filteredReports = useMemo(() => {
    ...
    return reports.filter((r) => {
      ...
      if (q && ![
        r.id, r.target_number, r.category,
        r.reporter_email ?? "", r.target_name ?? "",
        r.bank_name ?? "", r.store_name ?? "", r.suspect_city ?? "",
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto">
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Cari nomor, nama, toko, provinsi..."
          value={searchQuery}
      ...
      {selectedIds.size > 0 && (
        <div className="bg-slate-900 text-white rounded-2xl px-4 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <ActionBtn onClick={() => handleBulkAction("verified")} disabled={bulkLoading} loading={bulkLoading} icon={CheckCircle2} label="Approve" className="bg-emerald-500 hover:bg-emerald-400 text-white" />
            <ActionBtn onClick={() => handleBulkAction("rejected")} disabled={bulkLoading} loading={bulkLoading} icon={XCircle}     label="Reject"  className="bg-red-500 hover:bg-red-400 text-white" />
            <button onClick={() => setSelectedIds(new Set())} className="text-xs text-slate-400 hover:text-white px-2">Batal</button>
          </div>
        </div>
      )}
...
const CARD_ACTIONS: Record<string, { status: "verified" | "rejected" | "pending" | "withdrawn"; icon: React.ElementType; label: string; className: string }[]> = {
  pending:   [
    { status: "verified", icon: CheckCircle2, label: "Approve", className: "bg-emerald-500 text-white hover:bg-emerald-600" },
    { status: "rejected", icon: XCircle,      label: "Reject",  className: "bg-slate-100 text-slate-600 hover:bg-red-50 hover:text-red-500" },
  ],
  verified:  [{ status: "rejected", icon: XCircle,      label: "Reject",   className: "bg-slate-100 text-slate-500 hover:bg-red-50 hover:text-red-500" }],
  rejected:  [{ status: "verified", icon: CheckCircle2, label: "Approve",  className: "bg-slate-100 text-slate-500 hover:bg-emerald-50 hover:text-emerald-600" }],
  withdrawn: [{ status: "pending",  icon: Undo2,         label: "Restore",  className: "bg-slate-100 text-slate-500 hover:bg-amber-50 hover:text-amber-600" }],
};
...
function ReportCard({
  ...
  const actions       = CARD_ACTIONS[report.status] ?? [];
    ...
    <div className={`bg-white border rounded-2xl overflow-hidden transition-all ${isSelected ? "border-emerald-400 ring-1 ring-emerald-200" : "border-slate-200 hover:border-slate-300"}`}>
      <div className="px-3.5 py-3.5 sm:px-5">
        ...
        <div className="flex items-start gap-2.5">
          <input type="checkbox" checked={isSelected} onChange={onToggleSelect} className="w-4 h-4 accent-emerald-600 shrink-0 mt-1 rounded" />
          ...
          <div className="flex-grow min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-1.5 mb-1">
              ...
              {report.robot_score != null && (
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-semibold border ${ROBOT_SCORE_COLOR(report.robot_score)}`}>
                  <Bot className="w-2.5 h-2.5" />{report.robot_score}
                </span>
              )}
              {report.has_other_victims === "yes" && <Badge className="bg-orange-50 text-orange-600 border border-orange-200">Multi korban</Badge>}
              {relatedNumbers.length > 0 && <Badge className="bg-slate-100 text-slate-500">+{relatedNumbers.length} terkait</Badge>}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-slate-400">
              ...
              {report.loss_amount && <span className="text-red-500 font-semibold">{formatRupiah(report.loss_amount as number)}</span>}
              ...
              {report.store_name  && <span className="flex items-center gap-1 bg-slate-100 px-2 py-0.5 rounded-lg text-slate-500"><Store  className="w-2.5 h-2.5" />{report.store_name}</span>}
              {report.suspect_city && <span className="flex items-center gap-1 bg-slate-100 px-2 py-0.5 rounded-lg text-slate-500"><MapPin className="w-2.5 h-2.5" />{report.suspect_city}</span>}
              {evidenceUrls.length > 0 && <span className="flex items-center gap-1 bg-slate-100 px-2 py-0.5 rounded-lg text-slate-500"><Eye className="w-2.5 h-2.5" />{evidenceUrls.length} foto</span>}
            </div>
          ...
          <button onClick={onToggleExpand} className="w-8 h-8 bg-slate-100 rounded-xl flex items-center justify-center hover:bg-slate-200 transition-colors shrink-0">
        ...
        <div className="flex gap-2 mt-3 pl-[52px]">
          {actions.map((a) => (
            <ActionBtn
              key={a.status}
              onClick={() => onAction(report.id, a.status)}
              disabled={isLoading}
              loading={isLoading && a.status !== "rejected"}
              icon={a.icon}
              label={a.label}
...
function ReportDetail({
  ...
  relatedNumbers: any[];
  hasSocmed: boolean;
  hasReportedTo: boolean;
}) {
  const detailItems = [
    { label: "Pelapor",      value: report.reporter_email },
    report.bank_name     && { label: "Bank",          value: report.bank_name },
    report.loss_amount   && { label: "Kerugian",      value: formatRupiah(report.loss_amount as number), className: "text-red-600 font-semibold" },
    report.incident_date && { label: "Tgl Kejadian",  value: formatDateID(report.incident_date) },
    report.platform      && { label: "Platform",      value: report.platform },
    report.store_name    && { label: "Nama Toko",     value: report.store_name },
    report.suspect_city  && { label: "Provinsi",      value: report.suspect_city },
  ...
  return (
    <div className="border-t border-slate-100 px-3.5 sm:px-5 py-4 bg-slate-50/60 space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
        {detailItems.map((item, i) => <DetailItem key={i} label={item.label} value={item.value} className={item.className} />)}
        {report.link_url && (
      ...
      {relatedNumbers.length > 0 && (
        <div>
          <SectionLabel icon={Phone}>Nomor Terkait</SectionLabel>
      ...
      {hasSocmed && (
        <div>
          <SectionLabel icon={AtSign}>Akun Media Sosial</SectionLabel>
          <div className="flex flex-wrap gap-1.5">
      ...
      {hasReportedTo && (
        <div>
          <SectionLabel icon={ShieldAlert}>Sudah Lapor Ke</SectionLabel>
          <div className="flex flex-wrap gap-1.5">
      ...
      {/* Kronologi */}
      <div>
        <SectionLabel>Kronologi</SectionLabel>
      ...
      {report.robot_score != null && (
        <div>
          <SectionLabel icon={Bot}>Skor Robot</SectionLabel>
          <div className="bg-white border border-slate-100 rounded-xl p-3.5 space-y-3">
            <div className="flex items-center gap-3">
              <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all ${ROBOT_BAR_COLOR(report.robot_score)}`} style={{ width: `${report.robot_score}%` }} />
              </div>
              <span className={`text-sm font-bold tabular-nums w-8 text-right ${ROBOT_TEXT_COLOR(report.robot_score)}`}>{report.robot_score}</span>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ${ROBOT_SCORE_COLOR(report.robot_score)}`}>{report.robot_status ?? "pending"}</span>
            </div>
            {report.robot_reasons && report.robot_reasons.length > 0 && (
              <div className="space-y-1.5 pt-2 border-t border-slate-100">
                {report.robot_reasons.map((r: any, i: number) => (
      ...
      {evidenceUrls.length > 0 && (
        <div>
          <SectionLabel icon={Eye}>Bukti Foto ({evidenceUrls.length})</SectionLabel>
