...
function ActionMenu({
  report, loading, onVerify, onReject, onDetail, onDeleteClick,
}: {
  report: Report;
  loading: boolean;
  onVerify: () => void;
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-20">
          ...
          >
            <Eye className="w-3.5 h-3.5 text-slate-400" /> Lihat Detail
          </button>
          {report.status === 'pending' && (
            <>
              <button
                ...
                disabled={loading}
                className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-emerald-700 hover:bg-emerald-50 transition-colors disabled:opacity-40"
              >
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50 transition-colors disabled:opacity-40"
              >
          ...
          <button
            onClick={() => { onDeleteClick(); setOpen(false); }}
            className="w-full flex items-center gap-2 px-3 py-2 text-xs font-medium text-red-700 hover:bg-red-50 transition-colors"
          >
...
function DetailModal({ report, onClose, onVerify, onReject, loading }: {
  report: Report;
  onClose: () => void;
  onVerify: () => void;
  onReject: () => void;
  ...
  const targetValue  = (getField(report, 'targetValue',         'target_value')          as string)   ?? '';
  const targetType   = (getField(report, 'targetType',          'target_type')           as string)   ?? '';
  const targetName   = (getField(report, 'targetName',          'target_name')           as string)   ?? null;
  const bankName     = (getField(report, 'bankName',            'bank_name')             as string)   ?? null;
  const walletName   = (getField(report, 'walletName',          'wallet_name')           as string)   ?? null;
  const createdAt    = (getField(report, 'createdAt',           'created_at')            as string)   ?? '';
  const suspectPhoto = (getField(report, 'suspectPhotoUrl',     'suspect_photo_url')     as string)   ?? null;
  const storeName    = (getField(report, 'storeName',           'store_name')            as string)   ?? null;
  const suspectCity  = (getField(report, 'suspectCity',         'suspect_city')          as string)   ?? null;
  const socialMedia  = (getField(report, 'socialMediaAccounts', 'social_media_accounts') as string[]) ?? [];
  const reportedTo   = (getField(report, 'reportedTo',          'reported_to')           as string[]) ?? [];
  const hasVictims   = (getField(report, 'hasOtherVictims',     'has_other_victims')     as string)   ?? null;
  const evidenceUrls = (getField(report, 'evidenceUrls',        'evidence_urls')         as string[]) ?? [];
      ...
      >
        <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between">
          <div>
            ...
            <span className={`inline-flex mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ${status.className}`}>
              {status.label}
            </span>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          {(userEmail || userName) && (
            <div className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
              <div className="w-8 h-8 rounded-full bg-white border border-slate-200 flex items-center justify-center shrink-0">
                <span className="text-xs font-bold text-slate-500">{userName ? userName[0].toUpperCase() : '?'}</span>
              </div>
              <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs">
            <div>
              ...
              <p className="font-semibold text-slate-800">{TYPE_MAP[targetType]?.label ?? targetType}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-semibold text-slate-800">{formatDateID(createdAt)}</p>
            </div>
            {targetName && (
              <div>
            ...
            {report.amount && (
              <div>
                ...
                <p className="font-semibold text-red-600">{formatRupiah(report.amount)}</p>
              </div>
            )}
            {report.platform && (
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wide mb-0.5">Platform</p>
                <p className="font-semibold text-slate-800">{report.platform}</p>
              </div>
            )}
            {storeName && (
              <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {socialMedia.filter(Boolean).map((acc, i) => (
                  <span key={i} className="text-xs bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-full text-slate-700">{acc}</span>
                ))}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {reportedTo.filter(Boolean).map((to, i) => (
                  <span key={i} className="text-xs bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-full text-slate-700">{to}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {chronology && (
            <div>
              ...
              <p className="text-xs text-slate-700 leading-relaxed whitespace-pre-wrap bg-slate-50 border border-slate-200 rounded-xl p-3">
                {chronology}
              </p>
            </div>
          )}
          ...
          {suspectPhoto && (
            <div>
              ...
              <div className="relative w-20 h-20 rounded-lg overflow-hidden border border-slate-200">
                <Image src={suspectPhoto ?? ''} alt="Foto penipu" fill className="object-cover" unoptimized />
              </div>
            </div>
          )}
        ...
        {report.status === 'pending' && (
          <div className="sticky bottom-0 bg-white border-t border-slate-100 px-5 py-3 flex gap-2">
            <button onClick={onVerify} disabled={loading}
              className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl transition-all disabled:opacity-40">
              <CheckCircle className="w-3.5 h-3.5" /> Verifikasi
            </button>
            <button onClick={onReject} disabled={loading}
              className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white text-xs font-semibold rounded-xl transition-all disabled:opacity-40">
              <XCircle className="w-3.5 h-3.5" /> Tolak
            </button>
          </div>
        )}
...
function ConfirmDeleteModal({ report, onClose, onConfirm, loading }: {
  report: Report;
  onClose: () => void;
  onConfirm: () => void;
  loading: boolean;
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-sm p-5" onClick={e => e.stopPropagation()}>
        <div className="w-11 h-11 rounded-2xl bg-red-50 flex items-center justify-center mb-3">
          <AlertTriangle className="w-5 h-5 text-red-600" strokeWidth={2.25} />
        </div>
        <p className="text-sm font-black text-slate-900">Hapus laporan ini?</p>
        <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
          Laporan dengan nomor <span className="font-mono font-semibold text-slate-700">{targetValue}</span> beserta
          semua bukti yang terlampir akan dihapus permanen. Tindakan ini tidak bisa dibatalkan.
        </p>
        <div className="flex gap-2 mt-5">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 text-slate-600 text-xs font-semibold rounded-xl hover:bg-slate-100 transition-colors disabled:opacity-40"
          >
            Batal
          </button>
          <button
            onClick={onConfirm}
            disabled={loading}
            className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-xl transition-all disabled:opacity-40"
          >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <SummaryCard icon={FileText} iconBg="bg-emerald-50" iconColor="text-emerald-600" label="Total Laporan" value={totalCount} sub="Semua laporan masuk" />
        <SummaryCard icon={Clock} iconBg="bg-amber-50" iconColor="text-amber-600" label="Menunggu Verifikasi" value={pendingCount} sub="Belum diverifikasi" />
        <SummaryCard icon={ShieldCheck} iconBg="bg-emerald-50" iconColor="text-emerald-600" label="Terverifikasi" value={verifiedCount} sub="Telah diverifikasi" />
        <SummaryCard icon={XOctagon} iconBg="bg-rose-50" iconColor="text-rose-600" label="Ditolak" value={rejectedCount} sub="Laporan ditolak" />
      </div>
      ...
      {selected.size > 0 && (
        <div className="bg-slate-100 rounded-xl px-4 py-2.5 flex items-center gap-2 text-sm text-slate-600">
          ...
          <div className="ml-auto flex gap-2">
            ...
            <button onClick={clearSelection} className="px-3 py-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700 transition-colors">
              Batal
            </button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 text-left">
                <th className="px-4 py-3 w-10">
                  <input type="checkbox" checked={pageAllSelected} onChange={toggleSelectAll} className="w-4 h-4 accent-emerald-600" />
                </th>
                <th className="px-4 py-3 font-bold text-slate-700 text-xs">Nomor</th>
                <th className="px-4 py-3 font-bold text-slate-700 text-xs">Tipe</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center text-sm text-slate-400">Tidak ada laporan ditemukan.</td>
                </tr>
              ) : paginated.map(r => {
                ...
                return (
                  <tr key={r.id} className={`hover:bg-slate-50/60 transition-colors ${isSelected ? 'bg-emerald-50/40' : ''}`}>
                    <td className="px-4 py-3">
                      <input type="checkbox" checked={isSelected} onChange={() => toggleSelect(r.id)} className="w-4 h-4 accent-emerald-600" />
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-bold text-slate-900 font-mono text-xs">{targetValue}</p>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5 text-xs text-slate-600">
                        <TypeIcon className="w-3.5 h-3.5 text-slate-400" />
                        {typeMeta?.label ?? targetType}
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      {r.category ? (
                        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full border ${categoryClass(r.category)}`}>
                          {r.category}
                        </span>
                    ...
                    <td className="px-4 py-3 text-xs text-slate-500 hidden sm:table-cell">{formatDateID(createdAt)}</td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full border ${status.className}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <ActionMenu
                        ...
                        onDeleteClick={() => setDeleteTarget(r)}
                      />
                    </td>
                  </tr>
                );
        ...
        {filtered.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100">
            <p className="text-xs text-slate-500">
              Menampilkan {(currentPage - 1) * PER_PAGE + 1} - {Math.min(currentPage * PER_PAGE, filtered.length)} dari {filtered.length} laporan
            </p>
      ...
      {detailReport && (
        <DetailModal
          ...
          loading={loading === detailReport.id}
          onVerify={() => updateStatus(detailReport.id, 'verified')}
          onReject={() => updateStatus(detailReport.id, 'rejected')}
        />
      )}
