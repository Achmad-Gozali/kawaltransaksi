...
if data["success"]:
    ...
    print(f"Status: {status}")  # safe | warning | danger
...
if (data.success) {
  const { status, verified_reports, check_url } = data.data;
  const { requests_remaining } = data.meta;

  console.log("Status:", status); // safe | warning | danger
  console.log("Laporan terverifikasi:", verified_reports);
  console.log("Sisa request hari ini:", requests_remaining);
}`,
};
