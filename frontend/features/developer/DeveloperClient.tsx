...
import { Plus, RefreshCw, AlertTriangle, ChevronDown, Zap, Lock, List, PanelLeftClose, PanelLeftOpen, X } from 'lucide-react';
...
// Daftar isi dokumentasi -- id harus match dengan id di setiap <DocSection>
const DOC_NAV = [
  { id: 'overview',        label: 'Overview' },
  { id: 'quick-start',     label: 'Quick Start' },
  { id: 'autentikasi',     label: 'Autentikasi' },
  { id: 'environment',     label: 'Environment' },
  { id: 'endpoint-check',  label: 'Endpoint /check' },
  { id: 'format-response', label: 'Format Response' },
  { id: 'idempotency',     label: 'Idempotency' },
  { id: 'rate-limiting',   label: 'Rate Limiting' },
  { id: 'keamanan',        label: 'Keamanan API Key' },
    ...
    <aside className="hidden lg:block">
      <div className="sticky top-20 max-h-[calc(100vh-6rem)] overflow-y-auto pr-2">
        <div className="flex items-center justify-between px-1 mb-2">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Daftar Isi</p>
          <button onClick={onCollapse} title="Tutup navigasi" className="text-slate-300 hover:text-slate-500 transition-colors">
    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[70vh] overflow-y-auto p-4 pb-6">
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-bold text-slate-900">Daftar Isi</p>
          ...
          {DOC_NAV.map((item) => (
            <button
              ...
              className={`block w-full text-left px-3 py-2.5 text-sm rounded-lg transition-colors ${
                activeSection === item.id ? 'bg-emerald-50 text-emerald-700 font-bold' : 'text-slate-600 hover:bg-slate-50'
              }`}
            >
              {item.label}
            </button>
...
export default function DeveloperClient({ token, isLoggedIn }: Props) {
  ...
  const [activeSection, setActiveSection] = useState(DOC_NAV[0].id);
  ...
  // Scroll-spy: highlight section dokumentasi yang sedang aktif di viewport
  useEffect(() => {
  ...
  // Kunci scroll body selagi bottom-sheet mobile terbuka
  useEffect(() => {
  ...
  const goToSection = useCallback((id: string) => {
    ...
    const el = document.getElementById(id);
    if (!el) return;
    // Kalau section masih tertutup (accordion), buka dulu sebelum scroll
    const toggleBtn = el.querySelector('button[aria-expanded="false"]') as HTMLButtonElement | null;
    toggleBtn?.click();
            ...
            <div className="space-y-3 min-w-0">
              {!sidebarOpen && (
                <button
                  ...
                  className="hidden lg:flex items-center gap-1.5 text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors mb-2"
                >
  ...
  -H "X-API-Key: kt_live_your_key_here"`} />
              ...
              <DocSection id="autentikasi" title="Autentikasi">
                <p className="text-sm text-slate-600 leading-relaxed">
              ...
              <DocSection id="environment" title="Environment (Live vs Test)">
                <p className="text-sm text-slate-600 leading-relaxed">
              ...
              <DocSection id="endpoint-check" title="Endpoint -- GET /api/v1/check">
                <p className="text-sm text-slate-600 leading-relaxed">
              ...
              <DocSection id="format-response" title="Format Response">
                <p className="text-sm text-slate-600 leading-relaxed">
  ...
  "meta": {
    ...
    "expires_at": null
              ...
              <DocSection id="idempotency" title="Idempotency" defaultOpen={false}>
                <p className="text-sm text-slate-600 leading-relaxed">
  ...
  -H "Idempotency-Key: order-check-12345"`} />
              ...
              <DocSection id="keamanan" title="Keamanan API Key" defaultOpen={false}>
                <div className="space-y-3">
                  {[
              ...
              <DocSection id="best-practices" title="Best Practices" defaultOpen={false}>
                <div className="space-y-3">
                  {[
              ...
              <DocSection id="changelog" title="Changelog" defaultOpen={false}>
                <div className="space-y-4">
                  {[
                    {
