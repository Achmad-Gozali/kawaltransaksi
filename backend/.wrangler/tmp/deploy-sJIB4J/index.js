...
// src/lib/supabase.ts
function getSupabaseAdmin(env2) {
...
auth.post("/login", async (c) => {
  try {
    ...
    const { data: profile3, error: profileError } = await supabaseAdmin.from("profiles").select("id, failed_attempts, locked_until, is_banned").eq("email", normalizedEmail).single();
    if (profile3 && !profileError) {
      if (profile3.is_banned) {
        return c.json({ success: false, message: "Akun Anda telah dinonaktifkan. Hubungi admin." }, 403);
      }
      if (profile3.locked_until) {
        const lockedUntil = new Date(profile3.locked_until);
        if (lockedUntil > /* @__PURE__ */ new Date()) {
          ...
          return c.json({
            ...
            locked: true,
            locked_until: profile3.locked_until,
            remaining_ms: remainingMs
          }, 429);
        } else {
          await supabaseAdmin.from("profiles").update({ failed_attempts: 0, locked_until: null }).eq("id", profile3.id);
        }
      }
    }
    ...
    if (signInError) {
      ...
      const { data: freshProfile } = await supabaseAdmin.from("profiles").select("failed_attempts, locked_until, is_banned").eq("id", profile3.id).single();
      ...
      await supabaseAdmin.from("profiles").update({ failed_attempts: currentAttempts }).eq("id", profile3.id);
      if (remaining === 1) {
        return c.json({
          success: false,
    ...
    if (signInData.user) {
      const { data: freshProfile } = await supabaseAdmin.from("profiles").select("is_banned").eq("id", signInData.user.id).single();
      if (freshProfile?.is_banned) {
        await supabaseClient.auth.signOut();
        return c.json({ success: false, message: "Akun Anda telah dinonaktifkan. Hubungi admin." }, 403);
      }
      await supabaseAdmin.from("profiles").update({ failed_attempts: 0, locked_until: null }).eq("id", signInData.user.id);
    }
    return c.json({
      success: true,
...
var authMiddleware = createMiddleware(
  async (c, next) => {
    ...
    const { data: { user }, error: error3 } = await supabase.auth.getUser(token);
    if (error3 || !user) {
...
var VALID_TARGET_TYPES = ["phone", "bank_account", "ewallet"];
...
reports.post("/", authMiddleware, async (c) => {
  try {
    ...
    const { error: error3 } = await supabase.from("reports").insert({
      reporter_id: userId,
    ...
    if (error3) {
      console.error("Insert report error:", error3.message);
...
reports.post("/withdraw", authMiddleware, async (c) => {
  try {
    ...
    if (!reportId || !UUID_REGEX2.test(reportId)) {
      return c.json({ success: false, message: "ID laporan tidak valid." }, 400);
    }
    const supabase = getSupabaseAdmin(c.env);
    const { data: report2, error: fetchError } = await supabase.from("reports").select("id, status, reporter_id").eq("id", reportId).eq("reporter_id", userId).single();
    if (fetchError || !report2) return c.json({ success: false, message: "Laporan tidak ditemukan." }, 404);
    if (report2.status === "withdrawn") return c.json({ success: false, message: "Laporan sudah dalam status revisi." }, 400);
    const { error: error3 } = await supabase.from("reports").update({ status: "withdrawn" }).eq("id", reportId).eq("reporter_id", userId);
...
reports.put("/:reportId", authMiddleware, async (c) => {
  try {
    ...
    const { error: error3 } = await supabase.from("reports").update(sanitizedData).eq("id", reportId).eq("reporter_id", userId);
    if (error3) throw error3;
...
// src/routes/admin.ts
var admin = new Hono2();
var VALID_STATUSES = ["pending", "verified", "rejected", "withdrawn"];
var VALID_ROLES = ["user", "admin"];
async function requireAdmin(c, next) {
  const userId = c.get("userId");
  const supabase = getSupabaseAdmin(c.env);
  const { data: profile3 } = await supabase.from("profiles").select("role").eq("id", userId).single();
...
admin.get("/users", authMiddleware, requireAdmin, async (c) => {
  try {
    const supabase = getSupabaseAdmin(c.env);
    const { data: profiles } = await supabase.from("profiles").select("id, full_name, email, role, is_banned, created_at, updated_at").order("created_at", { ascending: false });
    const reportCounts = {};
...
admin.get("/users/:userId/banned", authMiddleware, requireAdmin, async (c) => {
  try {
    ...
    const supabase = getSupabaseAdmin(c.env);
    const { data: profile3 } = await supabase.from("profiles").select("is_banned").eq("id", userId).single();
    return c.json({ success: true, is_banned: profile3?.is_banned ?? false });
  } catch {
...
admin.patch("/reports/:id/status", authMiddleware, requireAdmin, async (c) => {
  try {
    ...
    const supabase = getSupabaseAdmin(c.env);
    const { error: error3 } = await supabase.from("reports").update({ status }).eq("id", id);
    if (error3) throw error3;
...
admin.patch("/users/:id/role", authMiddleware, requireAdmin, async (c) => {
  try {
    ...
    const supabase = getSupabaseAdmin(c.env);
    const { error: error3 } = await supabase.from("profiles").update({ role }).eq("id", id);
    if (error3) throw error3;
    return c.json({ success: true });
  } catch {
    return c.json({ success: false, message: "Terjadi kesalahan server." }, 500);
...
admin.patch("/users/:id/ban", authMiddleware, requireAdmin, async (c) => {
  try {
    ...
    const supabase = getSupabaseAdmin(c.env);
    const { error: error3 } = await supabase.from("profiles").update({ is_banned: true }).eq("id", id);
    if (error3) throw error3;
    return c.json({ success: true });
  } catch {
    return c.json({ success: false, message: "Terjadi kesalahan server." }, 500);
...
admin.patch("/users/:id/unban", authMiddleware, requireAdmin, async (c) => {
  try {
    const id = c.req.param("id");
    const supabase = getSupabaseAdmin(c.env);
    const { error: error3 } = await supabase.from("profiles").update({ is_banned: false }).eq("id", id);
    if (error3) throw error3;
    return c.json({ success: true });
  } catch {
    return c.json({ success: false, message: "Terjadi kesalahan server." }, 500);
