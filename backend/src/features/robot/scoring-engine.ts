...
export async function scoreReport(
  ...
  if (batcher) {
    ...
    await writeLog(logEntry, supabase).catch(() => {});
  }
...
export async function applyVerdict(
  ...
  options?: { ip?: string; kv?: KVNamespace; batcher?: LogBatcher }
): Promise<void> {
  ...
  // Jangan downgrade laporan yang sudah diverifikasi manual
  ...
  if (options?.batcher) {
    options.batcher.add(verdictLog);
  } else {
    await writeLog(verdictLog, supabase).catch(() => {});
  }
  ...
  // Auto-blocker — hanya jalan kalau ada KV yang valid
  if (result.verdict === 'rejected' && existingStatus !== 'verified' && options?.kv) {
    const { kv, ip = 'unknown' } = options;
    const reporterId = report.reporter_id ?? null;
    if (reporterId || ip !== 'unknown') {
      Promise.resolve(
        recordRejection(reporterId, ip, kv)
