...
const LIMITS = {
  ...
  LINK_URL: 500, CHRONOLOGY_MIN: 20, CHRONOLOGY_MAX: 5000,
  SOCIAL_ACCOUNT: 100, SOCIAL_ACCOUNTS_COUNT: 10,
  LOSS_AMOUNT_MAX: 999_999_999_999, STORE_NAME: 150,
};
...
const VALID_CATEGORIES = [
  ...
  'Donasi Palsu', 'Pura-pura Kecelakaan', 'Penipuan Umroh/Haji', 'Lainnya',
] as const;
...
reports.post('/', authMiddleware, async (c) => {
  try {
    ...
    // [AUTO-BLOCKER] Cek apakah user/IP diblokir
    if (c.env.LIMITER) {
    ...
    for (const [val, max, label] of fieldChecks) {
      if (val && String(val).length > max)
        return c.json({ success: false, message: `${label} terlalu panjang. Maks ${max} karakter.` }, 400);
    }
    ...
    if (Array.isArray(body.social_media_accounts)) {
      ...
      if (body.social_media_accounts.some((s: unknown) => typeof s === 'string' && s.length > LIMITS.SOCIAL_ACCOUNT))
        return c.json({ success: false, message: `Akun media sosial terlalu panjang. Maks ${LIMITS.SOCIAL_ACCOUNT} karakter.` }, 400);
    }

    // Rate limit & duplikat
    const rateLimit = await checkReportRateLimit(supabase, userId);
    if (rateLimit.blocked) return c.json({ success: false, message: rateLimit.reason }, 429);

    const cleanNumber = String(body.target_number).replace(/[^0-9]/g, '');
    const { count: dupCount } = await supabase.from('reports')
    ...
    // Sanitize
    const sanitizedChronology = sanitizeChronology(chronologyRaw);
    ...
    if (!cleanTargetNumbers.some((t: any) => t?.number === cleanNumber)) {
      cleanTargetNumbers.unshift({
        number: cleanNumber, type: body.target_type ?? 'phone',
    ...
    // Background: robot evaluasi + email + re-evaluate + auto-blocker
    c.executionCtx.waitUntil((async () => {
      try {
        ...
        if (freshReport) {
          ...
          // [AUTO-BLOCKER] Catat rejection & evaluasi block
          ...
          if ((finalStatus === 'verified' || finalStatus === 'rejected') && profile?.email) {
            await sendStatusEmail(supabase, userId, cleanNumber, reportId, finalStatus, c.env.RESEND_API_KEY);
          }
        }
        ...
        await Promise.all([
          profile?.email && sendReportCreatedEmail({
            ...
            targetNumber: cleanNumber, category: body.category,
            status: finalStatus === 'verified' ? 'verified' : 'pending',
            reportUrl, apiKey: c.env.RESEND_API_KEY,
          }).catch(err => console.error('[EMAIL] User:', err)),
          c.env.ADMIN_EMAIL && sendNewReportAdminEmail({
            adminEmail: c.env.ADMIN_EMAIL, reporterName: profile?.full_name ?? 'Pengguna',
            targetNumber: cleanNumber, category: body.category,
            status: finalStatus === 'verified' ? 'verified' : 'pending',
            reportUrl: 'https://kawaltransaksi.com/admin', apiKey: c.env.RESEND_API_KEY,
  ...
  try {
    ...
    if (Array.isArray(body.social_media_accounts)) {
      if (body.social_media_accounts.length > LIMITS.SOCIAL_ACCOUNTS_COUNT)
        return c.json({ success: false, message: `Maksimal ${LIMITS.SOCIAL_ACCOUNTS_COUNT} akun media sosial.` }, 400);
      if (body.social_media_accounts.some((s: unknown) => typeof s === 'string' && s.length > LIMITS.SOCIAL_ACCOUNT))
        return c.json({ success: false, message: 'Akun media sosial terlalu panjang.' }, 400);
    }
...
reports.get('/check-number/:number', async (c) => {
  try {
    ...
    // Hitung total laporan untuk nomor ini
